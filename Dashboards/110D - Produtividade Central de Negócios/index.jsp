                    ...
                    <head>
                     <!-- Querys -->
                        ...
                        <snk:query var="faturamento">
                            SELECT 'Faturamento' AS DESCRICAO, 
                                    'R$ ' || TO_CHAR(NVL(ROUND(SUM(VALOR),2),0), 'FM999G999G999D90') AS VALOR
                        ...
                        <snk:query var="devolucao">
                            SELECT 'Devolução' AS DESCRICAO, 
                                    'R$ ' || TO_CHAR(NVL(ROUND(SUM(VALOR),2),0), 'FM999G999G999D90') AS VALOR
                            FROM (
                            SELECT NVL(((ITE.VLRTOT - ITE.VLRDESC-ITE.VLRREPRED) * 
                              ...
                              INNER JOIN TGFVEN VEN
                                 ON VEN.CODVEND = CAB.AD_CODVEND  
                              WHERE TOP.GOLSINAL = -1 
                                AND DTFATUR BETWEEN :P_PERIODO.INI AND :P_PERIODO.FIN
                                AND CAB.STATUSNOTA = 'L'
                                AND VEN.ATIVO = 'S'
                                AND VEN.AD_HIERARQUIA IN (25, 30)
                                AND VEN.AD_CODCOR IN (:P_CODCOR)
                        ...
                        <snk:query var="prazomedio">
                          SELECT ROUND(AVG((SELECT AVG(PPG.PRAZO) AS PRAZO
                                              FROM TGFPPG PPG
                                             WHERE PPG.CODTIPVENDA = CAB.CODTIPVENDA)),0) AS PRAZO
                              FROM TGFCAB CAB
                             INNER JOIN  TGFITE ITE
                                ON ITE.NUNOTA = CAB.NUNOTA
                             INNER JOIN TGFTOP TOP 
                        ...
                        <snk:query var="positivacoes">
                          SELECT COUNT(DISTINCT CAB.NUNOTA) AS QTDREGISTROS  
                            FROM TGFCAB CAB
                           INNER JOIN  TGFITE ITE
                              ON ITE.NUNOTA = CAB.NUNOTA
                           INNER JOIN TGFTOP TOP 
